...
Zum Schuß noch ein Verzeichnis für die Daten erstellen und die Rechte anpassen:
...
Extraktion der config\_oai.xml:

```bash
unzip -p /opt/digiverso/viewer/bin/M2M.war WEB-INF/classes/config_oai.xml > /opt/digiverso/viewer/config/config_oai.xml
```

Anpassen der Einstellungen von "localhost:8080" auf die korrekte URL, damit die Resolver-Links funktionieren und sicherstellen, dass der Port des Tomcats auf dem der Solr Dienst läuft 8080 entspricht:
...
In der lokalen config\_viewer.xml muss der Block für die Benennung des RSS-Feeds individualisiert werden. Hier können individuelle Informationen hinterlegt werden
...
```bash
mysql -e "USE viewer;
INSERT INTO users (active,email,password_hash,score,superuser) VALUES (1,'goobi@intranda.com','$2a$10$IOW8.7geMec8SyHs5H.ld.aTTkH/IcRgfiLfP1dNy5Yh.I/Bd6D7O',0,1);"
```

Passwörter werden als bcrypt Hash gespeichet. Mit dem folgenden Kommando kann ein individueller Hash erzeugt werden. Das Passwort SECRETPASSWORD ist dafür anzupassen:

```bash
sudo apt install -y python3 python3-bcrypt
python3 -c 'import bcrypt; print(bcrypt.hashpw("SECRETPASSWORD".encode(), bcrypt.gensalt(10)))' | sed -e 's/\$2b\$/\$2a\$/g'
```
