...
Aktualisierung des Goobi viewer Indexers und Anpassen der Konfigurationsdatei `solr_indexerconfig.xml`. Wenn das Metadatum `MD_SERIESTITLE` existiert muss dieses in `MD_TITLE_SERIES` umbenannt werden. Siehe dazu auch den letzten Abschnitt in [Kapitel 6.2](../6/6.1-1.md).
