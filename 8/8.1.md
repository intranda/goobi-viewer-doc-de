...
Wird der TaskManager für die Generierung von PDF-Dateien verwendet muss ein UNIQUE Constraint auf die `id`Spalte in der `viewerdownloadjobs` Tabelle gesetzt werden. Dafür müssen gegebenenfalls vorher Duplikate entfernt werden:

```sql
SELECT * FROM viewerdownloadjobs WHERE id IN(SELECT id FROM viewerdownloadjobs GROUP BY id HAVING COUNT(id) > 1);

/*
DELETE FROM viewerdownloadjobs WHERE id=<one row from the duplicates>;
*/

ALTER TABLE viewerdownloadjobs ADD UNIQUE(id);
```
