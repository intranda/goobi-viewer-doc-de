...
{% hint style="danger" %}
Die Projektstrukur wurde geändert. Bestehende Themes sind nicht mehr mit dem aktuellen Goobi viewer Core kompatibel und müssen angepasst werden. Eine Anleitung erscheint in Kürze in Kapitel [8.2 Theme changes](8.2.md).
{% endhint %}

Wenn das Theme als [externes Theme](../2/2.16/2.16.1.md) eingebunden ist sind folgende Anpassungen notwendig:

Anpassen der Repository URL. Dafür in dem ausgecheckten Repository, zum Beispiel `/opt/digiverso/viewer/themes/goobi-viewer-theme-reference/` die Datei `.git/config/` editieren und die URL zum remote origin anpassen.
