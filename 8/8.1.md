...
Aktualisierung des Goobi viewer Indexers und Anpassen der Konfigurationsdatei `solr_indexerconfig.xml`. Wenn das Metadatum `MD_SERIESTITLE` existiert muss dieses in `MD_TITLE_SERIES` umbenannt werden. Siehe dazu auch den letzten Abschnitt in [Kapitel 6.2](../6/6.1.md).
...
Damit die vom Goobi viewer generierten [IIIF Presentation Manifeste](../2/2.33/2.33.3.md) von extern funktionieren, muss CORS erlaubt werde. Dafür das Headers Modul im Apache aktivieren sofern noch nicht geschehen:
