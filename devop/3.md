...
```bash
apt -y install git patch openjdk-11-jdk-headless tomcat9 mariadb-server apache2 ttf-mscorefonts-installer unzip zookeeperd
```
...
Zuerst wird die Applikation heruntergeladen. Danach werden die benötigten Dateien nach /opt/digiverso/indexer/ kopiert und dabei die indexerconfig\_solr.xml nach solr\_indexerconfig.xml umbenannt, der Tomcat Port kontrolliert und gegebenenfalls `C:` durch `/opt` ersetze. Zum Schluss wird die systemd Service Unit aktiviert:
...
wget -O /opt/digiverso/indexer/solr_indexerconfig.xml https://github.com/intranda/goobi-viewer-indexer/blob/master/goobi-viewer-indexer/src/main/resources/indexerconfig_solr.xml
...
Für die Installation der OAI und SRU Schnittstelle muss zuerst die Applikation an die erwartete Stelle im Dateisystem abgelegt werden:
...
Anschließend weitere benötigte Dateien herunterladen und die Rechte anpassen:
...
Danach die folgende Datei nach `/etc/tomcat9/Catalina/localhost/M2M.xml` legen: 
...
Optional lässt sich ein eigener Passwort-Hash wie folgt generieren:
