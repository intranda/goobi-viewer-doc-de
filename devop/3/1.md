...
</strong>wget https://archive.apache.org/dist/solr/solr/9.5.0/solr-9.5.0.tgz
...
Wenn in `/etc/zookeeper/conf/zoo.cfg` das `dataDir` nach `/tmp/` zeigt, `dataDir` auf einen persistenten Ort ändern, z.B. `dataDir=/var/lib/zookeeper-data`. Ordner anlegen und Berechtigungen anpassen: `mkdir /var/lib/zookeeper-data; chown zookeeper: /var/lib/zookeeper-data; chmod 750 /var/lib/zookeeper-data; systemctl restart zookeeper`
