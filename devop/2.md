...
Der Goobi viewer ist nun in der Lage sein Layout für von rechts nach links ausgerichtete Sprachen anzupassen. Aktuell werden hebräisch und arabisch Unterstützt. Das right-to-left Layout wird angewendet, sobald eine der beiden Sprachen über die Sprachauswahl gesetzt wird. Stark angepasste und individuelle Themes und CMS Templates unterstützen diese Funktion meist nicht vollständig ohne manuelle Korrekturen. Es muss in den Template Dateien im `<html>` Tag das Attribut `dir=""` in folgender Form gesetzt werden:
...
Aufgrund von Erweiterungen der Funktionalität ist das Styling einiger Widgets zu prüfen. Das _Widget Usage_ wurde um eine kopierbare Zitierempfehlung und ein Popover ergänzt \(JPG Download\). Das _Widget Download_ hat einen Infotext bekommen. Im _Widget User Interactions_ sind Funktionen zum Leeren des Caches hinzugetreten. Es sind daher teilweise Anpassungen in folgenden Dateien vorzunehmen:
...
In templateMirador.html muss der folgende Abschnitt:
