...
Außerdem wurde in der Konfigurationsdatei aufgeräumt und es wurde eine einheitliche Terminologie und Schreibweise für die Aktivierung und Anzeige von Funktionen eingeführt. Dafür wurden die Attribute `visible`, `display` oder `use` genauso wie potentielle Unterelemente wie `<enabled>true</enabled>` alle auf das Attribut `enabled="true"` vereinheitlicht. Die lokale Konfigurationsdatei muss hier zwingend geprüft und mit der globalen abgeglichen werden. Alle Änderungen hier in der Anleitung aufzulisten wäre deutlich zu aufwendig, hier aber ein paar Beispiele:
