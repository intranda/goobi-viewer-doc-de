...
FIXME Occurence of the splitting character in the string that separates the partner ID from the record identifier \(e.g. '1' means second occurrence\)
...
FIXME XPath expressions for image names/URLs relative to lideo:resourceSet. Used for automatic image downloads if \_downloadimages folder exists
