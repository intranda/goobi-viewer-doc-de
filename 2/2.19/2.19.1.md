...
</table>Unterhalb von einem `<param>` Element können `<replace />` Regeln definiert werden. Über die Attribute `string`, `char` oder `regex` lassen sich Werte definieren die aus dem Metadatum extrahiert und dann durch den im replace Block angegeben Wert ersetzt werden sollen. 
