...
Ähnlich funktioniert das Attribut `addCoordsToDocstruct`. Damit werden optional suchbare Geokoordinaten wie zum Beispiel die `WKT_*`_ _ _oder _`NORM_COORDS_*` Felder in das zugehörige Strukturelement-Doc anstelle des Page-Docs geschrieben. Standardwert ist `false`.
