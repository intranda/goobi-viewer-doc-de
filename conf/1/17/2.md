...
Felder wie `YEAR` können als `<field type="range">` definiert werden und werden dann in Form eines Sliders angezeigt. Über das optionale `style=""` Attribut mit den Werten `slider` und `graph` kann das Widget als klassischer Slider (default) oder die Suchtreffer als Graph angezeigt werden.
