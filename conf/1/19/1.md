...
Sind im Index gruppierte Metadaten vorhanden, die wiederum weitere gruppierte Metadaten beinhalten (siehe [Kapitel 2.5](../../2/5.md#mehrstufige-gruppierte-metadaten)), können innerhalb eines `<metadata>` Elements weitere `<metadata>` Elemente konfiguriert werden, die diese abbilden (theoretisch beliebig tief).
