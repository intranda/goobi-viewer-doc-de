...
Die Konfigurationsdatei `config_crowdsourcing.xml` befindet sich in dem [lokalen Konfigurationsordner](../2.-konfiguration-core/2.3-lokale-konfigurationsdateien.md) neben den anderen Konfigurationsdateien.Der Pfad dorthin wird in der `config_viewer.xml` definiert und muss den vollen Pfad zur Datei enthalten:
